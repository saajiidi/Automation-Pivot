...
def render_sales_trends(df: pd.DataFrame):
    ...
    st.plotly_chart(px.line(daily, x="order_day", y="Revenue", title="Daily Revenue", markers=True).update_layout(height=350), width="stretch")
    c1, c2 = st.columns(2)
    ...
    with c1: st.plotly_chart(px.bar(by_day, x="day_name", y="Orders", title="Orders by Day", color="Orders").update_layout(height=320), width="stretch")
    with c2:
        ...
        st.plotly_chart(px.imshow(pivot.values, x=list(range(24)), y=["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"], title="Activity Heatmap").update_layout(height=320), width="stretch")
